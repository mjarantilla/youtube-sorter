...
def log(msg, silent=False):
    try:
        ...
        print(log_msg.encode('utf-8'), file=log_fp)
        if not silent:
...
def get_client():
    ...
    handler = YoutubeClientHandler(pickle='sorter.pickle')
        ...
        if tier != '1':
            ...
            for vid_details in autolist[tier]:
                if vid_details['videoId'] not in filler_ids_added:
                    ...
                    log("Added to Watch Later as filler: %s: %s" % (vid_details['channelTitle'], vid_details['videoTitle']))
        ...
        for item in autolist:
            ...
            if item['videoId'] not in already_added_ids:
                ...
                if item['sourcePlaylistId'] == target_playlist_id:
                    ...
                    try:
                        msg_list = msg_list + ["\tMoving into the", playlist_label, "playlist in position ", str(pos), ":"]
                        body = {
